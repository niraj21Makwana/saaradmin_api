...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_fasalMP` WHERE `status`='Approved'");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4>Fasal Muawja Patrak List (total Approved <span class="notify-badge"><?php echo $total_con; ?></span>)</h4>
...
function loadData(page){
    $.ajax({
        type: "POST",
         data:{page_no:page},
        url: "ajax/fetch-data/fetch-fasalMuvajaPApproved-data",
        success: function (response) {
