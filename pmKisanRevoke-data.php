...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_PMSamanNidhi`");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4> पी एम किसान Revoke  List (Total Data <?php echo $total_con; ?>)</h4>
          
        <?php
   }
   ?>
                    <div class="card-header-action">
                        <a href="ajax/export/export-pmKisanRevoke-data.php" class="btn btn-icon icon-left btn-primary col-12"><i class="fas fa-download"></i>Download xls File</a>
...
function loadData(page){
    $.ajax({
        type: "POST",
        data:{page_no:page},
        url: "ajax/fetch-data/fetch-pmKisanRevoke-data",  
        success: function (response) {
...
$(document).on("click",".dlt-btn", function () {
    var onPMSamanIdDel = $(this).closest('tr').find('#PMSamanIdEdit').text();
    // alert(onSliderDelt);
    var deleteTypeIs = "Single";
    var onPMSamanIdDeltID = "onPMSamanIdDeltID";
    var url = "delete-pmKisanRevoke-data";
...
$("#dlt-selected").on('click', function () {
    var onPMSamanId  = [];
    var pageUrl = "delete-pmKisanRevoke-data";
