...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_nabalikSeBalik`");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4>नाबालिग से बालिग   List (Total Data <?php echo $total_con; ?>)</h4>
          
        <?php
   }
   ?>
                    <div class="card-header-action">
                        <a href="ajax/export/export-nabalikSeBalik-data.php" class="btn btn-icon icon-left btn-primary col-12"><i class="fas fa-download"></i>Download xls File</a>
...
function loadData(page){
    $.ajax({
        type: "POST",
        data:{page_no:page},
        url: "ajax/fetch-data/fetch-nabalikSeBalik-data",
...
$(document).on("click",".dlt-btn", function () {
    var onNasebaIdDel = $(this).closest('tr').find('#nasebaIdEdit').text();
    // alert(onSliderDelt);
    var deleteTypeIs = "Single";
    var onNasebaIdDeltID = "onNasebaIdDeltID";
    var url = "delete-nabalikSeBalik-data";
    deleteDataAjax(url,onNasebaIdDel,onNasebaIdDeltID,deleteTypeIs,function(){
            loadData();
    });
});
...
$("#dlt-selected").on('click', function () {
    var onNasebaId = [];
    var pageUrl = "delete-nabalikSeBalik-data";
    var keyDataName = "onNasebaIdKey";
    var deleteTypeIs = "Multiple";
    $(":checkbox:checked").each(function(key){
        onNasebaId[key] = $(this).val();
    });
    // alert(onSliderId);
    multiDeleteData(pageUrl,keyDataName,onNasebaId,deleteTypeIs,function(){
            loadData();
        });
