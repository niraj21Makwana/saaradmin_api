...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_prman_patra`");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4> प्रमाण पत्र  List (Total Data <?php echo $total_con; ?>)</h4>
...
function loadData(page){
    $.ajax({
        type: "POST",
        data:{page_no:page},
        url: "ajax/fetch-data/fetch-pramanPatra-data",
...
$(document).on("click",".dlt-btn", function () {
    var onPrpatraIdDel = $(this).closest('tr').find('#prpatraIdEdit').text();
    // alert(onSliderDelt);
    var deleteTypeIs = "Single";
    var onPrpatraIdDeltID = "onPrpatraIdDeltID";
    var url = "delete-pramanPatra-data";
    deleteDataAjax(url,onPrpatraIdDel,onPrpatraIdDeltID,deleteTypeIs,function(){
            loadData();
    });
});
...
$("#dlt-selected").on('click', function () {
    var onPrpatraId  = [];
    var pageUrl = "delete-pramanPatra-data";
    var keyDataName = "onPrpatraIdKey";
    var deleteTypeIs = "Multiple";
    $(":checkbox:checked").each(function(key){
        onPrpatraId[key] = $(this).val();
    });
    // alert(onSliderId);
    multiDeleteData(pageUrl,keyDataName,onPrpatraId,deleteTypeIs,function(){
            loadData();
        });
});               
