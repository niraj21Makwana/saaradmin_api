...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_kabjaVsimankan`");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4>कब्जा/सीमांकन List (Total Data <?php echo $total_con; ?>)</h4>
          
        <?php
   }
   ?>
                    <div class="card-header-action">
                        <a href="ajax/export/export-kabjaVsimankan-data.php" class="btn btn-icon icon-left btn-primary col-12"><i class="fas fa-download"></i>Download xls File</a>
...
function loadData(page){
    $.ajax({
        type: "POST",
        data:{page_no:page},
        url: "ajax/fetch-data/fetch-kabjaVsimankan-data",
        success: function (response) {
...
$(document).on("click",".dlt-btn", function () {
    var onkabjaVSIdDel = $(this).closest('tr').find('#kabjaVSIdEdit').text();
    // alert(onSliderDelt);
    var deleteTypeIs = "Single";
    var onkabjaVSIdDeltID = "onkabjaVSIdDeltID";
    var url = "delete-kabjaVsimankan-data";
    deleteDataAjax(url,onkabjaVSIdDel,onkabjaVSIdDeltID,deleteTypeIs,function(){
            loadData();
    });
});
...
$("#dlt-selected").on('click', function () {
    var onkabjaVSId = [];
    var pageUrl = "delete-kabjaVsimankan-data";
    var keyDataName = "onkabjaVSIdKey";
    var deleteTypeIs = "Multiple";
    $(":checkbox:checked").each(function(key){
        onkabjaVSId[key] = $(this).val();
    });
    // alert(onSliderId);
    multiDeleteData(pageUrl,keyDataName,onkabjaVSId,deleteTypeIs,function(){
            loadData();
        });
