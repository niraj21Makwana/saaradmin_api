...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_kalamBara`");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4> कॉलम 12  List (Total Data <?php echo $total_con; ?>)</h4>
          
        <?php
   }
   ?>
                    <div class="card-header-action">
                        <a href="ajax/export/export-kalamBara-data.php" class="btn btn-icon icon-left btn-primary col-12"><i class="fas fa-download"></i>Download xls File</a>
...
function loadData(page){
    $.ajax({
        type: "POST",
        data:{page_no:page},
        url: "ajax/fetch-data/fetch-kalamBara-data",
...
$(document).on("click",".dlt-btn", function () {
    var onKalamBaraIdDel = $(this).closest('tr').find('#kalamBaraIdEdit').text();
    // alert(onSliderDelt);
    var deleteTypeIs = "Single";
    var onKalamBaraIdDeltID = "onKalamBaraIdDeltID";
    var url = "delete-kalamBara-data";
    deleteDataAjax(url,onKalamBaraIdDel,onKalamBaraIdDeltID,deleteTypeIs,function(){
            loadData();
    });
});
...
$("#dlt-selected").on('click', function () {
    var onKalamBaraId  = [];
    var pageUrl = "delete-kalamBara-data";
    var keyDataName = "onKalamBaraIdKey";
    var deleteTypeIs = "Multiple";
    $(":checkbox:checked").each(function(key){
        onKalamBaraId[key] = $(this).val();
    });
    // alert(onSliderId);
    multiDeleteData(pageUrl,keyDataName,onKalamBaraId,deleteTypeIs,function(){
            loadData();
        });
});               
