...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_diversion_aavedan` WHERE `status`='Pending'");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4>Diversion Aavedan List (total pending <span class="notify-badge"><?php echo $total_con; ?></span>)</h4>
                      ...
                      <table class="table table-striped table-hover" id="tableExport" style="width:100%;">
                        <thead>
                          <tr>
                            ...
                            <th>क्र</th>
                            <th>आवेदन जिसके नाम से डायवर्सन(व्यपवर्तित ) करना हे</th>
                            <th>आवेदक का पता</th>
                            ...
                            <th>सर्व नंबर</th>
                            <th>रकबा</th>
                            <th>कितना डायवर्सन(व्यपवर्तित ) वर्ग मीटर </th>
                            <th>प्रयोजन</th>
                            <th>मोबाइल नंबर </th>
                            <th>EMAIL id</th>
                            <th>आधार कार्ड</th>
...
// end here
  $(document).on('change', '#getstatus', function() {
       ...
       $.ajax({
        type: "POST",
        url: "ajax/update-data/update-diversionAavedan-status",
...
function loadData(page){
    $.ajax({
        type: "POST",
         data:{page_no:page},
        url: "ajax/fetch-data/fetch-diversionAavedan-data",
        success: function (response) {
...
$(document).on("click",".dlt-btn", function () {
    var ondiversionAvedanIdDel = $(this).closest('tr').find('#diversionAvedanIdEdit').text();
    // alert(onSliderDelt);
    var deleteTypeIs = "Single";
    var ondiversionAvedanIdDeltID = "ondiversionAvedanIdDeltID";
    var url = "delete-DiversionAavedan-data";
    deleteDataAjax(url,ondiversionAvedanIdDel,ondiversionAvedanIdDeltID,deleteTypeIs,function(){
            loadData();
    });
});
...
$("#dlt-selected").on('click', function () {
    var onDiversionAvedanId = [];
    var pageUrl = "delete-DiversionAavedan-data";
    var keyDataName = "onDiversionAvedanIdKey";
    var deleteTypeIs = "Multiple";
    $(":checkbox:checked").each(function(key){
        onDiversionAvedanId[key] = $(this).val();
    });
    // alert(onSliderId);
    multiDeleteData(pageUrl,keyDataName,onDiversionAvedanId,deleteTypeIs,function(){
            loadData();
        });
});               
...
.then((willDelete) => {
  if(willDelete){ 
    $.ajax({
        type: "POST",
        url: "ajax/delete-data/delete-DiversionAavedan-data",
      success: function (response) {
      if(response==1){
