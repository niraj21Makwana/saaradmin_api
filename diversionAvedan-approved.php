...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_diversion_aavedan` WHERE `status`='Approved'");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4>Diversion Aavedan List (total Approved <span class="notify-badge"><?php echo $total_con; ?></span>)</h4>
...
function loadData(page){
    $.ajax({
        type: "POST",
         data:{page_no:page},
        url: "ajax/fetch-data/fetch-diversionAavedanApproved-data",
        success: function (response) {
