...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_mritak_namantran`");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4>मृतक नामान्तरण List (Total Data <?php echo $total_con; ?>)</h4>
          
        <?php
   }
   ?>
                    <div class="card-header-action">
                        <a href="ajax/export/export-mrtatakNamantran-data.php" class="btn btn-icon icon-left btn-primary col-12"><i class="fas fa-download"></i>Download xls File</a>
...
function loadData(page){
    $.ajax({
        type: "POST",
        data:{page_no:page},
        url: "ajax/fetch-data/fetch-mrtatakNamantran-data",
        success: function (response) {
...
$(document).on("click",".dlt-btn", function () {
    var onMritakIdDel = $(this).closest('tr').find('#mritakIdEdit').text();
    // alert(onSliderDelt);
    var deleteTypeIs = "Single";
    var onMritakIdDeltID = "onMritakIdDeltID";
    var url = "delete-mrtatakNamantran-data";
    deleteDataAjax(url,onMritakIdDel,onMritakIdDeltID,deleteTypeIs,function(){
            loadData();
    });
});
...
$("#dlt-selected").on('click', function () {
    var onMritakId = [];
    var pageUrl = "delete-mrtatakNamantran-data";
    var keyDataName = "onMritakIdKey";
    var deleteTypeIs = "Multiple";
    $(":checkbox:checked").each(function(key){
        onMritakId[key] = $(this).val();
    });
    // alert(onSliderId);
    multiDeleteData(pageUrl,keyDataName,onMritakId,deleteTypeIs,function(){
            loadData();
        });
});              
// ===============================
