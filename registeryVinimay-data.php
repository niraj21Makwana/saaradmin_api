...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_reg_vinamay`");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4>रजिस्ट्री विनिमय List (Total Data <?php echo $total_con; ?>)</h4>
          
        <?php
   }
   ?>
                    <div class="card-header-action">
                        <a href="ajax/export/export-registeryVinimay-data.php" class="btn btn-icon icon-left btn-primary col-12"><i class="fas fa-download"></i>Download xls File</a>
...
function loadData(page){
    $.ajax({
        type: "POST",
        data:{page_no:page},
        url: "ajax/fetch-data/fetch-registeryVinimay-data",  
        success: function (response) {
...
$(document).on("click",".dlt-btn", function () {
    var onRegVinamayIdDel = $(this).closest('tr').find('#regVinamayIdEdit').text();
    // alert(onSliderDelt);
    var deleteTypeIs = "Single";
    var onRegVinamayIdDeltID = "onRegVinamayIdDeltID";
    var url = "delete-registeryVinimay-data";
    deleteDataAjax(url,onRegVinamayIdDel,onRegVinamayIdDeltID,deleteTypeIs,function(){
            loadData();
    });
});
...
$("#dlt-selected").on('click', function () {
    var onRegVinamayId  = [];
    var pageUrl = "delete-registeryVinimay-data";
    var keyDataName = "onRegVinamayIdKey";
    var deleteTypeIs = "Multiple";
    $(":checkbox:checked").each(function(key){
        onRegVinamayId[key] = $(this).val();
    });
    // alert(onSliderId);
    multiDeleteData(pageUrl,keyDataName,onRegVinamayId,deleteTypeIs,function(){
            loadData();
        });
});               
