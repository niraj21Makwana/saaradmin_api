...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_batankan`");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4>भूमि बटांकन List (Total Data <?php echo $total_con; ?>)</h4>
          
        <?php
   }
   ?>
                    <div class="card-header-action">
                        <a href="ajax/export/export-batankan-data.php" class="btn btn-icon icon-left btn-primary col-12"><i class="fas fa-download"></i>Download xls File</a>
...
function loadData(page){
    $.ajax({
        type: "POST",
         data:{page_no:page},
        url: "ajax/fetch-data/fetch-batankan-data",
        success: function (response) {
...
$(document).on("click",".dlt-btn", function () {
    var onbatankanIdDel = $(this).closest('tr').find('#batankanIdEdit').text();
    // alert(onSliderDelt);
    var deleteTypeIs = "Single";
    var onbatankanIdDeltID = "onbatankanIdDeltID";
    var url = "delete-batankan-data";
    deleteDataAjax(url,onbatankanIdDel,onbatankanIdDeltID,deleteTypeIs,function(){
            loadData();
    });
});
...
$("#dlt-selected").on('click', function () {
    var onbatankanId = [];
    var pageUrl = "delete-batankan-data";
    var keyDataName = "onbatankanIdKey";
    var deleteTypeIs = "Multiple";
    $(":checkbox:checked").each(function(key){
        onbatankanId[key] = $(this).val();
    });
    // alert(onSliderId);
    multiDeleteData(pageUrl,keyDataName,onbatankanId,deleteTypeIs,function(){
            loadData();
        });
});               
