        ...
        if($countUserId==1){
            ...
            if($userGovermentId!="" || $userName!=""){
                ...
                if($userGovermentId!=""){
                    ...
                    $updateUserdata = mysqli_query($con,"UPDATE `saarUsersAuth` SET `userName`='$userName' WHERE `userId`='$userId'");
                        if($updateUserdata){
                            // if query run
                            ...
                            // if count > 0
                             while($getAllData = mysqli_fetch_assoc($updatedUDataReturn)){
                                ...
                                $response = array('status'=>array('code'=>SUCCESS,'message'=>'success'),'msg'=>'Your Name has been Updated Successfully !','response'=>$responseData);
                                print_r(json_encode($response));
                            }else{
                                // if count < 1
                                $response = array('status'=>array('code'=>DATA_NOT_FOUND,'message'=>'data not Found'),'response'=>$responseData);
                 ...
                 // Empty data error
                $response = array('status'=>array('code'=>EMPTY_DATA,'message'=>'Empty data'),'msg'=>'Please Fill Data !','response'=>$responseData);
                print_r(json_encode($response));
            }  
