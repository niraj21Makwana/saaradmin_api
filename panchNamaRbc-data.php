...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_panchnamaRBC`");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4> पंचनामा RBC List (Total Data <?php echo $total_con; ?>)</h4>
          
        <?php
   }
   ?>
                    <div class="card-header-action">
                        <a href="ajax/export/export-panchNamaRbc-data.php" class="btn btn-icon icon-left btn-primary col-12"><i class="fas fa-download"></i>Download xls File</a>
...
function loadData(page){
    $.ajax({
        type: "POST",
        data:{page_no:page},
        url: "ajax/fetch-data/fetch-panchNamaRbc-data",
...
$(document).on("click",".dlt-btn", function () {
    var onPanRBCIdDel = $(this).closest('tr').find('#panRBCIdEdit').text();
    // alert(onSliderDelt);
    var deleteTypeIs = "Single";
    var onPanRBCIdDeltID = "onPanRBCIdDeltID";
    var url = "delete-panchNamaRbc-data";
    deleteDataAjax(url,onPanRBCIdDel,onPanRBCIdDeltID,deleteTypeIs,function(){
            loadData();
    });
});
...
$("#dlt-selected").on('click', function () {
    var onPanRBCId  = [];
    var pageUrl = "delete-panchNamaRbc-data";
    var keyDataName = "onPanRBCIdKey";
    var deleteTypeIs = "Multiple";
    $(":checkbox:checked").each(function(key){
        onPanRBCId[key] = $(this).val();
    });
    // alert(onSliderId);
    multiDeleteData(pageUrl,keyDataName,onPanRBCId,deleteTypeIs,function(){
            loadData();
        });
});               
