...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_atikraman`");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4>अतिक्रमण List (Total Data <?php echo $total_con; ?>)</h4>
          
        <?php
   }
   ?>
                    <div class="card-header-action">
                        <a href="ajax/export/export-atikraman-data.php" class="btn btn-icon icon-left btn-primary col-12"><i class="fas fa-download"></i>Download xls File</a>
...
function loadData(page){
    $.ajax({
        type: "POST",
         data:{page_no:page},
        url: "ajax/fetch-data/fetch-atikraman-data",
        success: function (response) {
...
$(document).on("click",".dlt-btn", function () {
    var onAtikramanIdDel = $(this).closest('tr').find('#atikramanIdEdit').text();
    // alert(onSliderDelt);
    var deleteTypeIs = "Single";
    var onAtikramanIdDeltID = "onAtikramanIdDeltID";
    var url = "delete-atikraman-data";
    deleteDataAjax(url,onAtikramanIdDel,onAtikramanIdDeltID,deleteTypeIs,function(){
            loadData();
    });
});
...
$("#dlt-selected").on('click', function () {
    var onAtikramanId = [];
    var pageUrl = "delete-atikraman-data";
    var keyDataName = "onAtikramanIdKey";
    var deleteTypeIs = "Multiple";
    $(":checkbox:checked").each(function(key){
        onAtikramanId[key] = $(this).val();
    });
    // alert(onSliderId);
    multiDeleteData(pageUrl,keyDataName,onAtikramanId,deleteTypeIs,function(){
            loadData();
        });
});               
