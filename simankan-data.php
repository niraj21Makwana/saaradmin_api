...
<style>
 ...
 <?php
   $mob_dire = mysqli_query($con,"SELECT * FROM `saar_simankan`");
   $total_con = mysqli_num_rows($mob_dire);
   if($total_con>0 || $total_con==0){
        ?>
                    <h4> सीमांकन  List (Total Data <?php echo $total_con; ?>)</h4>
          
        <?php
   }
   ?>
                    <div class="card-header-action">
                        <a href="ajax/export/export-simankan-data.php" class="btn btn-icon icon-left btn-primary col-12"><i class="fas fa-download"></i>Download xls File</a>
...
function loadData(page){
    $.ajax({
        type: "POST",
        data:{page_no:page},
        url: "ajax/fetch-data/fetch-simankan-data",
...
$(document).on("click",".dlt-btn", function () {
    var onSimankanIdDel = $(this).closest('tr').find('#simankanIdEdit').text();
    // alert(onSliderDelt);
    var deleteTypeIs = "Single";
    var onSimankanIdDeltID = "onSimankanIdDeltID";
    var url = "delete-simankan-data";
    deleteDataAjax(url,onSimankanIdDel,onSimankanIdDeltID,deleteTypeIs,function(){
            loadData();
    });
});
...
$("#dlt-selected").on('click', function () {
    var onSimankanId  = [];
    var pageUrl = "delete-simankan-data";
    var keyDataName = "onSimankanIdKey";
    var deleteTypeIs = "Multiple";
    $(":checkbox:checked").each(function(key){
        onSimankanId[key] = $(this).val();
    });
    // alert(onSliderId);
    multiDeleteData(pageUrl,keyDataName,onSimankanId,deleteTypeIs,function(){
            loadData();
        });
});               
